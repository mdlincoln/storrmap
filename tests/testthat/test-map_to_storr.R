...
test_that("input and keys of different length throws error", {
  expect_error(
    map_to_storr(x = letters,
                 storr = test_storr,
                 output_keys = c("a", "b"),
                 .f = toupper), regexp = "length")
})
...
test_that("adding exisitng key in same namespace causes error", {
  expect_error(
    map_to_storr(x = "foo",
                 storr = test_storr,
                 output_keys = "2",
                 .f = toupper), regexp = "already exist")
})
...
test_that("mapping writes new keys", {
  ...
  expect_equivalent(
    map_to_storr(x = letters,
                 storr = test_storr,
                 output_keys = newkeys,
                 .f = toupper), c("75f1160e72554f4270c809f041c7a776", "3a5505c06543876fe45598b5e5e5195d",
